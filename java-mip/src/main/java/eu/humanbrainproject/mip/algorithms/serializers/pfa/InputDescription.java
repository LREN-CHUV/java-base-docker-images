...
public abstract class InputDescription<T extends Algorithm> {
      ...
      case CATEGORICAL_STRING:
        {
          ...
          Schema enumType =
              SchemaBuilder.builder().enumeration("Enum_" + covariable).symbols(symbols);
          if (nullableField) {
            schema =
                SchemaBuilder.unionOf().type(enumType).and().intType().and().nullType().endUnion();
          } else {
            schema = enumType;
          }
          break;
  ...
  public void writePfaOutput(JsonGenerator jgen) throws Exception {
    ...
    if (getVariables().length == 1) {
      ...
      jgen.writeStringField("doc", "Output is the list of estimations of output variables");
    ...
    jgen.writeObjectFieldStart("query");
    {
        ...
        jgen.writeArrayFieldStart("fields");
        {
          ...
          jgen.writeStartObject();
          {
            if (getVariables().length == 1) {
              ...
              jgen.writeStringField("doc", "List of dependent variables");
              jgen.writeStringField("name", "variables");
              jgen.writeObjectFieldStart("type");
              {
      ...
      jgen.writeObjectFieldStart("init");
      {
        if (getVariables().length == 1) {
          ...
          jgen.writeObjectField("variables", this.getVariables());
        }
  ...
  public static String toPFAType(VariableType variableType) {
    switch (variableType) {
      ...
      case CATEGORICAL_INT:
        return "int";
      default:
        throw new IllegalArgumentException("Unknown type: " + variableType);
    }
  }
}
