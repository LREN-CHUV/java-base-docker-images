...
[ $(git tag --points-at HEAD | grep java-mip | wc -l) == 1 ] || (
...
mvn deploy:deploy-file \
  "-Durl=https://api.bintray.com/maven/hbpmedical/maven/eu.humanbrainproject.mip.algorithms:adapter/;publish=1" \
   -DrepositoryId=bintray-hbpmedical-maven -Dfile=target/mip-adapter-for-deploy.jar -DpomFile=pom.xml
./tests/test.sh
echo "[ok] Done"
...
# Notify Microbadger
curl -XPOST https://hooks.microbadger.com/images/hbpmip/java-mip/s7FIv3BMcBkyKpj0sIDJSNRrrfg=
